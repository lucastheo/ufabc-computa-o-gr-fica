...
void OpenGLWigget::createShaders(){
    ...
    if( !resultadoCompilacaoShader( vertexShader ) ){
        qDebug("Erro em abrir o Vertex shader");
    ...
    if( !resultadoCompilacaoShader( fragementShader ) ){
        qDebug("Erro em abrir o Fragment shader");
...
void OpenGLWigget::createVBOs(){
    ...
    for( GLuint i = 0; i < vertices1.size(); i++ ){
        colors1[ i ] = QVector4D(1-float(i)/float( vertices1.size()),float(i)/float( vertices1.size()),1,1);
    }
    for( GLuint i = 0; i < vertices1.size(); i++ ){
        indices1[ i*2    ] = i;
        indices1[ i*2 + 1] = i+1;
    ...
    for( GLuint i = 0; i < vertices2.size(); i++ ){
        colors2[ i ] = QVector4D(float(1)/float(i),0,0.5,1);
    }

    for( GLuint i = 0; i < indices2.size(); i++ ){
        indices2[ i ] = (i+1)%vertices2.size();
    }
    indices2[indices2.size()-1]=0;
    ...
    vertices3.resize(8);
    colors3.resize(vertices3.size());
    indices3.resize(vertices3.size()*2 + 2);
    ...
    for( GLuint i = 0; i < vertices3.size(); i++ ){
        colors3[ i ] = QVector4D(1,0,1-float(1)/float(i),1);
    }
    indices3[ 0 ] = 7;
    indices3[ 1 ] = 0;
